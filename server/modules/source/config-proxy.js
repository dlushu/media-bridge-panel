 ...
 *
 * ⚠️ **前端默认已不走这里**：配置中心页的 iframe **直连源自己的地址**
 * （`http://<访问面板用的域名>:<源端口>/website`）—— 地址栏就能看出看的是哪个源，
 * 也不用面板在中间记。这条代理退成**兜底**，两种情况才需要：
 *   · 源端口没发布到宿主（浏览器连不上源）；
 *   · 用 https 打开面板（浏览器会拦 http 的 iframe）。
 * 直连时下面这套"记哪个源"的机制**用不上**；兜底时它照旧生效。
 *
 * ## 为什么需要它
