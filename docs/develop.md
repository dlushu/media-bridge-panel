...
- 阈值与条数存在 `agg.json`（`matchMinScore` 默认 0.85 / `matchMaxItems` 默认 8 / `matchExtraK` 默认 0，
  可在「聚合设置」页修改），请求里可覆盖（web「聚合搜索」页的那三个输入框即对应它们）。
...
- **接续补打 = 前面一条能用的都没拿到时才兜底往下打**（见 [ADR-0027](adr/0027-extra-fetch-only-when-zero.md)，
  ...
  日志一行示例：
  `↻ agg 接续补打：阶段一 8 条里能用 0 条 → 往下打了 8 条（上限 8 条），拿到 1 条能用的`。
